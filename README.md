...
Для нагрузочного тестирования испольщую gutling, ибо другие инструменты хз

Уменьшид кост для скорости авторизации

Кря походу этот nginx не нужен

долго выбирал как нагр тестирование делать

как делать реплицирование бд, если везде требуется доступ на чтение.

при нагр тестировании было выявлено, что балансировка нагрузки при метода api/inf на отдельный инстанс который подключен к реплецируемой бд явлеется менее рациональной, чем балансировка между двемя узлами, подключенных к одной бд

100к юзеров слишком много, выдерживает только 3к

почему не использовал with в методах репозитория - долгооо

проблема с миграцией для e2e

жетско тупанул. надо было без айди писать бд. Было бы быстрее
